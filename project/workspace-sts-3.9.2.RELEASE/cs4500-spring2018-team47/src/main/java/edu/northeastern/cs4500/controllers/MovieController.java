...
public class MovieController {
	...
	try {
		...
		try {
		    localDbConnector.addMovieIntoMovieList(userId, "Browse History", movie.get("imdbID"),
			    movie.get("title"), formatter.format(new Date()));
		} catch (SQLException sq) {
		    logger.error(sq.getMessage());
		}
	...
	int communityRating;
	try {
	    communityRating = localDbConnector.getCommnunityMovieRating(movie.get("imdbID"));
	    if (communityRating != 0) {
		modelAndView.addObject("communityRating", communityRating);
	    } else {
		modelAndView.addObject("communityRating", "Unrated");
	    }
	} catch (SQLException e) {
